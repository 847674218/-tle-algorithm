...
public class SimilarityMatrix implements Serializable, Cloneable {
        ...
        if (links.containsKey(targetArtifactId) == false) {
            ...
            if (score != links.get(targetArtifactId)) {
                throw new IllegalArgumentException(String.format(
                        "Link for source artifact %s and target artifact %s has already been added to the spare matrix",
                        sourceArtifactId,
                        targetArtifactId));
            }
        }
    ...
    public Map<String, Double> getLinksForSourceId(String sourceArtifactId) {
        ...
        // System.out.println(SimilarityMatrix.class.getName()+"/getLinksForSourceId/candidate
        // trceability matrix/line167: "+s);
        // }
    ...
    public boolean isLinkInVariableCut(String sourceArtifactId, String targetArtifactId) {
        ...
        threshold = 0.0;
        LinkedHashMap<String, Double> links = (LinkedHashMap) matrix.get(sourceArtifactId);
        int cutNumInVariableCut = (int) (getCountOfLinksAboveThresholdForSourceArtifact(sourceArtifactId)
                * variableCut);
        if (links != null) {
    ...
    public LinksList getLinksAboveVariableThreshold() {
        ...
        for (String sourceArtifactId : matrix.keySet()) {
            LinksList linksAboveVariableThresholdForSourceId = getLinksAboveVariableThresholdForSourceArtifact(
                    sourceArtifactId);
    ...
    public LinksList getLinksAboveScaleThreshold() {
        ...
        for (String sourceArtifactId : matrix.keySet()) {
            LinksList linksAboveScaleThresholdForSourceId = getLinksAboveScaleThresholdForSourceArtifact(
                    sourceArtifactId);
    ...
    public LinksList getQualityLinks() {
        ...
        for (String source : sourceArtifactsIds()) {
            ...
            // System.out.println(" qualityTarget = " + source + " " + qualityTarget + " " +
            // baseScore);
        }
