...
// public SimilarityMatrix Compute(ArtifactsCollection source,
// ArtifactsCollection target) {
// /**
// * @date 2018.1.26
// * @author zzf
...
// // k = 100;
// System.out.println(target.size() + projectName + " lsi_k-------------------"
// + LSI_K);
...
// return Compute(new TermDocumentMatrix(source), new
// TermDocumentMatrix(target), new TermDocumentMatrix(bothSourceAndTarget));
// }

// private SimilarityMatrix Compute(TermDocumentMatrix source,
// TermDocumentMatrix target, TermDocumentMatrix both) {
...
// TermDocumentMatrix sourceWithTFIDF = ReplaceIDWithTFIDF(sourceIDs,
// TFIDF_svd);
// TermDocumentMatrix targetWithTFIDF = ReplaceIDWithTFIDF(targetIDs,
// TFIDF_svd);
...
// TermDocumentMatrix tfidf_svd =
// convertRealMatrixToTermDocumentMatrix(rebuildMatrix, tfidf_origin);
// return tfidf_svd;
// }

// private RealMatrix convertTermDocumentMatrixToRealMatrix(TermDocumentMatrix
// tfidf_origin) {
// double[][] dates = new
// double[tfidf_origin.NumTerms()][tfidf_origin.NumDocs()];
...
// private TermDocumentMatrix convertRealMatrixToTermDocumentMatrix(RealMatrix
// rebuildMatrix, TermDocumentMatrix tfidf_origin) {
...
// private TermDocumentMatrix ReplaceIDWithTFIDF(TermDocumentMatrix ids,
// TermDocumentMatrix tfidf) {
// for (int i = 0; i < ids.NumDocs(); i++) {
// for (int j = 0; j < ids.NumTerms(); j++) {
// ids.setValue(i, j, tfidf.getValue(ids.getDocumentName(i),
// ids.getTermName(j)));
// }
// }
// return ids;
...
// private SimilarityMatrix ComputeSimilarities(TermDocumentMatrix ids,
// TermDocumentMatrix tfidf) {
...
// if (cross == 0.0) {
// links.add(new SingleLink(ids.getDocumentName(i).trim(),
// tfidf.getDocumentName(j).trim(), 0.0));
// } else {
// links.add(new SingleLink(ids.getDocumentName(i), tfidf.getDocumentName(j),
// product / cross));
// }
// }
...
// sims.addLink(link.getSourceArtifactId(), link.getTargetArtifactId(),
// link.getScore());
// }
// }
